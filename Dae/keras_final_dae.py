...
for train_index, test_index in skf.split(train_data, train_target):
    ...
    model.fit(x1,y1, batch_size=128, validation_data=(x2, y2), verbose=1, epochs=1, callbacks=cb)
                     
    model = load_model('keras_final_dae.h5')

    train_all[test_index] = model.predict(x2)[:,0]

    test_all[:,i] = model.predict(test_data)[:,0]

    # test_all[:,i] = model.predict(train_data)[:,0]

    score = roc_auc_score(y2, train_all[test_index])
...
filename_save = 'keras_final_test.csv'

print("Saving {}".format(filename_save))

df1['Criminal'] = np.mean(test_all,axis=1)
df1[['PERID','Criminal']].to_csv(filename_save, index=False, float_format='%.5f')

df1['Criminal'] = train_all
df1[['Criminal']].to_csv('keras_final_train.csv', index=False, float_format='%.5f')
